...
export class StorageService {
  ...
  constructor(private readonly userService: UserService) {
    this.smartStockCache = cache({database: 'smartstock', collection: 'config'});
  }

  async saveSales(batchs: BatchModel[]): Promise<any> {
    const activeShop = await this.userService.getCurrentShop();
    await cache({database: 'sales', collection: activeShop.projectId})
  ...
  async removeStocks(): Promise<any> {
    ...
    return await cache({database: 'stocks', collection: shop.projectId}).clearAll();
  }

  async getStocks(): Promise<StockModel[]> {
    const shop = await this.userService.getCurrentShop();
    const stocksCache = cache({database: 'stocks', collection: shop.projectId});
